...
| `jijing_overseas` | 极境「海外用户」开关（`"true"` / `"false"`）。开启后所有极境请求改走 `global.jjowo.com` 香港线路；前端 `buildProxyUrl` 与 Rust `resolve_base_url` 都从此 key 读取。真相源:`src/providers/jijing/baseUrl.ts` | 前端设置保存 |
