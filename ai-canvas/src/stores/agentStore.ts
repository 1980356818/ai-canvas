...
export const useAgentStore = create<AgentState>((set, get) => ({
  ...
  async sendMessage(projectId, content, model) {
    ...
    const defaultRef = await modelService.getDefaultChatModel();
    const resolvedModel = model ?? defaultRef.modelId;
    const provider = modelService.resolveProvider(resolvedModel, defaultRef.providerId);
