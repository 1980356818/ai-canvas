...
//
// 旧 scheduleBackgroundSave / cancelBackgroundSave 已删除:按 cardId 单飞 + 只补
// 单字段的设计救不回批量第 2..N 张、也不补 results[].url(显示层优先读 results,
// 导致"补救成功但徽标/远端图还在")。统一替代品见 lib/mediaLocalize.ts
// (scheduleCardMediaLocalization / sweepProjectMediaLocalization)。
